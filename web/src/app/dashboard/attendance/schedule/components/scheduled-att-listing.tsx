...
import { useDeleteScheduleAttendance, useScheduledCustomAttendanceBase } from '@/hooks/query/useAttQ'
...
function ScheduledAttListing() {
  ...
  const {mutate:dlt,isPending:deleting} = useDeleteScheduleAttendance()
  ...
  if (isPending){
    return    <Card>
        <div className="flex items-center justify-center my-4">
          ...
          {
            !module  ? 
            <p className='text-muted-foreground'> Select a module to get the scheduled attendances</p>
            :
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Registered on {moment(record.created_at).format("dddd DD-MMMM-YYYY ")}
                          </p>
                        </div>
                      </div>

                      <Button onClick={()=>dlt(record.id)} disabled={deleting}
                        variant="destructive"
                        size="sm"
                      >
                        {  deleting ? <ServerRequestLoader size={16} stroke={2}/> : <Trash2 className="h-4 w-4" /> }
                      </Button>
                    </div>
                  ))}
                </div>
